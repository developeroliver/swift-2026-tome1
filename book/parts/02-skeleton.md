...
Qu'est-ce que Swift ? Philosophie du langage, Swift moderne, compilation, Swift Package Manager, Xcode / Swift Playground, premier programme Swift.

## 2. Syntaxe de base {#chap-2}

Commentaires, instructions, blocs de code, identifiants, conventions Swift, `print()`.

# Partie 2 — Les fondamentaux {#partie-2}

## 3. Variables et constantes {#chap-3}

`let`, `var`, mutabilité, inférence de type, annotation de type, type safety.
...
Arithmétiques, comparaison, logiques, affectation, `+=`/`-=`, ternaire, opérateurs personnalisés.

## 6. Conditions {#chap-6}

`if`, `else`, `else if`, `switch`, pattern matching, `where`.

## 7. Boucles {#chap-7}

`for`, `for-in`, `while`, `repeat-while`, `break`, `continue`.
...
Premier projet complet en ligne de commande : boucle de jeu, entrées utilisateur, conditions, ranges.
